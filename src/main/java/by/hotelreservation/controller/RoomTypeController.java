...
public class RoomTypeController {
    ...
    @Autowired
    @Qualifier(value = "roomTypeService")
    private CrudServiceExtended<RoomType> roomTypeService;

    @RequestMapping(method = RequestMethod.GET, produces = "application/json")
    public List<RoomType> getAll(){
        List<RoomType> resultList = null;
        try {
            resultList = (List<RoomType>) roomTypeService.getAll();
    ...
    public List<EntityDto> getAllHeaders(){
        ...
        try {
            resultList = roomTypeService.getAllHeaders();
        }catch (ServiceException e){
            logger.error(e);
        }
        return resultList;
